...
period_input = 10    #note that this period should not exceed the max range specified earlier
...
#Check if the asset weights sum up to 100%
w_check <- non_us_equity_w + us_equity_w + us_bond_w    

#Calculate cumulative returns and excess cumulative returns
ppd_cum_returns <- ppd_select %>% 
  group_by(plan_name) %>% 
  mutate(opt_benchmark = benchmark_portfolio(return, acwi_exUS, IWV, VBTIX),
         opt_benchmark_return = opt_benchmark[[1]][2]*acwi_exUS + opt_benchmark[[1]][3]*IWV + opt_benchmark[[1]][4]*VBTIX,
         custom_benchmark_return = non_us_equity_w * acwi_exUS + us_equity_w * IWV + us_bond_w * VBTIX,
         across(c(return, opt_benchmark_return, custom_benchmark_return), ~ cumprod(1+ .x), .names = "cum_{.col}")) %>%      #calculate cumulative returns across three variables
  ...
  mutate(fy = ifelse(is.na(fy), min(fy, na.rm = T) - 1, fy),              #add the "base" fy 
         ...
         cum_custom_benchmark_excess = cum_return - cum_custom_benchmark_return
  ) %>% 
  ungroup()
...
ppd_avg_returns <- ppd_cum_returns %>%
  group_by(plan_name) %>%
  summarise(across(c(return, opt_benchmark_return, custom_benchmark_return), ~ geo_return(.x, na.rm = T), .names = "avg_{.col}")) %>% 
  ungroup() %>% 
  mutate(avg_opt_benchmark_excess = avg_return - avg_opt_benchmark_return,
         avg_custom_benchmark_excess = avg_return - avg_custom_benchmark_return)
  


#Get 2020 AALs to map to the size of visualized dots
ppd_avg_returns_final <- ppd_select %>% 
  filter(fy == 2020, !is.na(aal)) %>% 
  select(fy:aal) %>% 
